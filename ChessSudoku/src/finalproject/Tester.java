...
class veryEasy3x3_solution implements Runnable {
	...
	public void run() {
		...
		int puzzleSize = 0;
		try {
			puzzleSize = ChessSudokuTry.readInteger(in);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
class medium3x3_12solutions_solution implements Runnable {
	...
	public void run() {
		...
		// Test if the final Puzzle is the same
		int counter = 0;
		for (ChessSudokuTry cur : s.solutions) {
			counter++;
	...
	public void run() {
		...
		for (int i = 0; i < puzzles.length; i++) {
			...
			try {
				...
				int puzzleSize = ChessSudokuTry.readInteger(in);
...
class ChessSudoku_extra_methods implements Runnable {
	@Override
	public void run() {
		Class<ChessSudokuTry> cls = ChessSudokuTry.class;
...
class ChessSudoku_extra_constructors implements Runnable {
	...
	public TConstructor[] getRequiredConstructors() {
		...
		String name = ChessSudokuTry.class.toString().split(" ")[1];
...
public class Tester {
	...
	public static boolean isSolved(ChessSudokuTry puzzle, boolean knightRule,
	...
	public static boolean isValidValue(ChessSudokuTry puzzle, int row, int col,
	...
	public static long runSolve(ChessSudokuTry puzzle, boolean allSolutions,
			long timeoutMillis)
